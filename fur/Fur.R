library(trackViewer)
setwd('/home/phaneuf/ros_tol_gdv/fur')


feats <- read.csv(file = "./fur_feats.csv", stringsAsFactors = FALSE)
aa_chain <- feats[feats$feature == "Chain", ]
feats <- feats[feats$feature != "Chain", ]

muts <- read.csv(file = "./fur_ale_muts.csv", stringsAsFactors = FALSE)
...
# x11(width = 6.5, height = 6.5)  # Will output to a separate window; good for testing
pdf("Fur.pdf", width=11, height=4)
lolliplot(mutations,
          features,
          legend = legend,
