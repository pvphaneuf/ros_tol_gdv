library(trackViewer)
setwd('/home/phaneuf/ros_tol_gdv/oxyR')


feats <- read.csv(file = "./oxyR_feats.csv", stringsAsFactors = FALSE)
aa_chain <- feats[feats$feature == "Chain", ]
feats <- feats[feats$feature != "Chain", ]

muts <- read.csv(file = "./oxyR_ale_muts.csv", stringsAsFactors = FALSE)
...
# x11(width = 16.25, height = 8)  # Will output to a separate window; good for testing
pdf("OxyR.pdf", width=16.25, height=8)
lolliplot(mutations,
          features,
          legend = legend,
