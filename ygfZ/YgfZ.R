library(trackViewer)
setwd('/home/phaneuf/ros_tol_gdv/ygfZ')


feats <- read.csv(file = "./ygfZ_feats.csv", stringsAsFactors = FALSE)
aa_chain <- feats[feats$feature == "Chain", ]
feats <- feats[feats$feature != "Chain", ]

muts <- read.csv(file = "./ygfZ_ale_muts.csv", stringsAsFactors = FALSE)
...
# x11(width = 8, height = 8)  # Will output to a separate window; good for testing
pdf("YgfZ.pdf", width=8, height=8)
lolliplot(mutations,
          features,
          legend = legend,
